...
describe RowMatcher do 
  ...
  describe 'calculate_direct_match' do
    let(:action){ calculate_direct_match(row, source_record) }

    context 'with match' do
      before(:each) do
        fields[2] = source_record['doctor']['npi']
      end
      ...
      context 'with multiple matching fields' do
        ...
        it 'does not log additional matching fields' do
          expect(action['fields']).to eq(['npi'])
          expect(action['source']).to eq(source_record)
        end
      end
